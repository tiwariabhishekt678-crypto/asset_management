...
const useAssetData = (isOtherAssetPage) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const url = isOtherAssetPage !== undefined 
        ? `${REACT_BASE_URL}/assets?is_other_asset=${isOtherAssetPage}`
        : `${REACT_BASE_URL}/assets?category=hardware`;
      ...
      const enrichedData = data.map((item) => ({
        ...
        assetTypeCategory: item.asset_type ? item.asset_type.category : 'N/A',
        assignTo: item.assigned_to ? item.assigned_to.full_name : 'Unassigned',
        assignedToId: item.assigned_to ? item.assigned_to.id : null,          // <-- current employee UUID
        lastAssignedToId: item.last_assigned_to ? item.last_assigned_to.id : null, // <-- last employee UUID
        ...
        received: 'Yes',
        brand: item.asset_type ? item.asset_type.name : 'N/A',
        whoAssigned: item.assigned_by ? item.assigned_by.name : 'N/A',        // <-- real assigner name
        assignedByUserId: item.assigned_by ? item.assigned_by.id : null,       // <-- user UUID for logs API
        tech_specs: item.tech_specs || {},
        invoices: item.invoices || [],
        invoiceNo: item.invoices && item.invoices.length > 0 ? item.invoices.map(inv => inv.invoice_number).join(', ') : 'N/A',
        price: item.invoices && item.invoices.length > 0 ? item.invoices.reduce((sum, inv) => sum + (parseFloat(inv.asset_cost) || 0), 0) : 0,
        assetHistory: [
          {
            action: 'Created',
            by: 'Admin',
...
function AssetManagement({ isOtherAssetPage = false }) {
  ...
  const handleAddAsset = useCallback(
    (values) => {
      const newAsset = {
        ...values,
        key: `A${String(cardData.length + 1).padStart(3, '0')}`,
        assetId: `A${String(cardData.length + 1).padStart(3, '0')}`,
        tech_specs: {},
        assetHistory: [
          {
            action: 'Added',
            by: 'Current User',
            to: values.assignTo || null,
            date: new Date().toISOString().split('T')[0],
            location: 'Unknown',
          },
        ],
      };
      setCardData((prev) => [...prev, newAsset]);
      closeModal('add');
      addForm.resetFields();
    },
    [cardData.length, setCardData, closeModal, addForm]
  );
        ...
        if (result.isConfirmed) {
          try {
            ...
            const response = await fetch(
              `${REACT_BASE_URL}/asset/${record.assetId || record.id}/delete`,
              { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }
            );
            if (!response.ok) {
              const data = await response.json();
              throw new Error(data.detail || data.message || 'Failed to delete asset');
            }
            setCardData((prev) =>
            ...
            if (errorMessage.includes('corrupt asset action log') || errorMessage.includes('Corrupt log id')) {
              const logIdMatch = errorMessage.match(/Corrupt log id:\s*([0-9a-fA-F-]{36})/);
              const logId = logIdMatch ? logIdMatch[1] : null;
              const result = await Swal.fire({
                title: 'Cleanup corrupted action log entry?',
              ...
              if (result.isConfirmed && logId) {
                try {
                  const token =
                    localStorage.getItem('access_token') || sessionStorage.getItem('access_token');
                  const cleanupResponse = await fetch(
                    `${REACT_BASE_URL}/asset-logs/cleanup/${logId}`,
                    { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } }
                  );
                  if (!cleanupResponse.ok) {
                    const cleanupData = await cleanupResponse.json().catch(() => ({}));
                    throw new Error(cleanupData.detail || cleanupData.message || 'Failed to clean corrupt log entry');
                  }
                  Swal.fire('Cleaned!', 'The corrupt action log entry has been removed. Please retry deletion.', 'success');
                } catch (cleanupError) {
                  Swal.fire('Error!', cleanupError.message || 'Failed to clean corrupt log entry', 'error');
                }
              }
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          try {
            const token =
              localStorage.getItem('access_token') || sessionStorage.getItem('access_token');
            const response = await fetch(`${REACT_BASE_URL}/asset-assignment/unassign`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
              },
              body: JSON.stringify({ asset_id: record.assetId, location, remarks }),
            });
            if (!response.ok) {
              const data = await response.json();
              throw new Error(data.message || 'Failed to unassign asset');
            }
            setCardData((prev) =>
              prev.map((item) =>
    ...
    async (record, newStatus) => {
      ...
      if (formValues) {
        try {
          ...
          const response = await fetch(`${REACT_BASE_URL}/asset/${record.assetId}/update`, {
            method: 'PUT',
            headers: { Authorization: `Bearer ${token}` },
            body: formData,
          });
          ...
          if (!response.ok) {
            const data = await response.json();
            throw new Error(data.message || data.detail || 'Failed to update asset status');
          }
    ...
    const actionsColumn = {
      ...
      render: (_, record) => (
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          {record.assetStatus !== 'decommissioned' && (
  ...
  const handleExport = async (format) => {
    ...
    try {
      const exportUrl =
        format === 'excel' && !isOtherAssetPage
          ? `${REACT_BASE_URL}/reports/assets/bgis-hardware-excel`
          : `${REACT_BASE_URL}/reports/assets/${format}`;
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `${format === 'excel' && !isOtherAssetPage ? 'BGIS_hardware' : 'assets_report'}.${format === 'excel' ? 'xlsx' : 'pdf'}`;
      ...
      {activeView === 'assetManagement' ? (
        <>
          ...
          <Tabs defaultActiveKey="home" activeKey={activeTabKey} onChange={setActiveTabKey} className="asset-tabs">
            ...
            <TabPane
              tab={<span><img src="/icons/editassettype.svg" alt="Asset Types" style={{ width: 16, marginRight: 8 }} />Asset Types</span>}
              key="assetTypes"
            >
              <AssetTypeTab 
          ...
          />
          <AssetInfoModal visible={modalState.info} onCancel={() => closeModal('info')} infoRecord={selectedRecord} />
        ...
        <EditAssetForm onBack={() => { fetchData(); setActiveView('assetManagement'); }} editRecord={selectedRecord} isOtherAssetPage={isOtherAssetPage} />
      ) : null}
    </div>
  );
}
